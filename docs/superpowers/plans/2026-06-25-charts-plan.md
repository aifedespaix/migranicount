...
export function averageIntensityByDay(
  ...
  return result.map(({ day, total, count }) => ({
    day,
    avg: count === 0 ? 0 : Math.round((total / count) * 10) / 10,
  }));
}
...
export function averageIntensityByWeek(
  ...
  return result.map(({ week, total, count }) => ({
    week,
    avg: count === 0 ? 0 : Math.round((total / count) * 10) / 10,
  }));
}
...
<div class="charts-grid">
  <button class="chart-card" @click="openDetail('frequency')">
    ...
    <div class="chart-wrap">
      <FrequencyChart :migraines="migraines.migraines" :period="period" />
    </div>
  </button>
  <button class="chart-card" @click="openDetail('intensity')">
    <h2>Intensité moyenne</h2>
    <div class="chart-wrap">
      <IntensityChart :migraines="migraines.migraines" :period="period" />
    </div>
  </button>
</div>
```
