...
import { defineStore } from 'pinia'
import { ref } from 'vue'
import { listMedocsFavoris, registerMedocUsage } from '../storage/migraineRepository'
...
```vue
<template>
  <button class="fab" @click="$emit('click')" aria-label="Ajouter une migraine">+</button>
</template>
...
<template>
  <div class="step">
    ...
    <label>Heure de début <input type="time" v-model="model.heureDebut" /></label>
    <label>
...
<template>
  <div class="step">
    <h2>Intensité</h2>
    <input type="range" min="1" max="10" v-model.number="model.intensite" :style="{ accentColor: trackColor }" />
...
<template>
  <div class="step">
    <h2>Notes</h2>
    <textarea v-model="model.notes" rows="5" placeholder="Notes libres (optionnel)"></textarea>
  </div>
</template>
...
<template>
  <div class="step">
    ...
    <ul class="recap-list">
      ...
      <li>Début : {{ model.heureDebut }} — Fin : {{ model.heureFin ?? 'en cours' }}</li>
      <li>Intensité : {{ model.intensite }} / 10</li>
      <li>Médicaments : {{ model.medocs.map(m => `${m.nom} (${m.heure})`).join(', ') || 'aucun' }}</li>
      <li>Avortée : {{ model.avortee ? 'oui' : 'non' }}</li>
      <li>Nausée : {{ model.nausee ? 'oui' : 'non' }} — Vomissement : {{ model.vomissement ? 'oui' : 'non' }}</li>
...
function confirmImport() {
  ...
  migraines.migraines = migraines.getById ? [...migraines.migraines] : migraines.migraines
  location.reload()
}
</script>
```
